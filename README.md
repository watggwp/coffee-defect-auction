...
| ไฟล์ | หน้าที่ |
|---|---|
| `App.tsx` | hash router (`#/` หน้าแรก, `#/lot/<id>` หน้าล็อต, `#/admin` ตั้งค่า) + View Transitions, Header, Toast provider, ชื่อผู้เสนอราคาที่ใช้ร่วมกัน |
| `api.ts` | ชนิดข้อมูล, REST, WebSocket, admin login/logout/token/reset |
| `hooks/useAuction.ts` | state กลาง: ล็อต กติกา สถานะเชื่อมต่อ เหตุการณ์ (มี lotId/ผู้เสนอก่อน-หลัง) และการเสนอราคา |
| `pages/AuctionPage.tsx` | หน้าแรก: การ์ดสรุปทุกล็อต |
| `pages/LotPage.tsx` | หน้าล็อตเดียว: ผลตรวจ เสนอราคา timeline เหตุการณ์ของล็อตนั้น |
| `pages/AdminPage.tsx` | หน้า login admin, แผงตั้งค่ากติกา, โซนลบข้อมูล |
| `components/LotCard.tsx` | การ์ดสรุป + ฟอร์มเสนอราคา + ฟังก์ชันคำนวณเวลา, `CountdownRing.tsx` วงแหวนนับถอยหลัง, `Header.tsx` แถบบน + ปุ่มสลับธีม, `Toasts.tsx` แจ้งเตือน |
| `index.css` | design tokens (สี, มุม, easing) ธีมสว่าง/มืด, keyframes |
| `App.css` | สไตล์ทุกส่วน |

### 6.1 หน้าแรก (`#/`) การ์ดสรุปทุกล็อต

หน้าแรกเป็น "กระดานสรุป" อย่างเดียว ไม่มีปุ่มเสนอราคา แต่ละล็อตเป็นการ์ดที่กดเข้าไปได้

| ส่วน | ทำอะไร |
|---|---|
| แถบบน | โลโก้, เมนู ประมูล / Admin, สถานะเชื่อมต่อ (จุดเขียวกระพริบ = สด), ปุ่มสลับธีมสว่าง/มืด |
| Hero | สรุปกติกาปัจจุบัน + ตัวเลข กำลังประมูล / คุณนำอยู่ / คุณชนะ |
| แถบเครื่องมือ | ช่องชื่อผู้เสนอราคา (ใช้ร่วมทุกหน้า จำใน localStorage ของ browser นั้น), แท็บ กำลังประมูล (เรียงใกล้ปิดก่อน) / ปิดแล้ว |
| การ์ดล็อต | เลขล็อต, คลาสหลัก, จำนวนรายการตรวจ, **วงแหวนนับถอยหลัง**, ชิปสถานะ/ต่อเวลา/คุณนำอยู่, ราคาปัจจุบัน (เด้งเมื่อเปลี่ยน), ผู้เสนอสูงสุด, ปุ่ม "เข้าไปเสนอราคา →" |
| Toast | ล็อตใหม่, กติกาเปลี่ยน, ล้างข้อมูล และของคุณเอง: ถูกแซง / ชนะ |

### 6.2 หน้าล็อต (`#/lot/<id>`) เสนอราคาและเหตุการณ์ของล็อตนั้น

กดการ์ดแล้วเข้ามาหน้านี้ (เปลี่ยนหน้าแบบนุ่ม ๆ ด้วย View Transitions API ถ้า browser รองรับ)

| ส่วน | ทำอะไร |
|---|---|
| ส่วนหัว | เลขล็อต คลาสหลัก กล้อง เวลาตรวจ/ปิด วงแหวนใหญ่ ชิปสถานะ ต่อเวลา คุณนำอยู่ / คุณถูกแซง |
| ผลตรวจ AI | ทุกรายการในล็อตพร้อม track_id และแถบความมั่นใจ และเหตุผลราคาเริ่ม |
| แผงเสนอราคา | ราคาปัจจุบันตัวใหญ่, ผู้นำ, ช่องชื่อ, ปุ่มราคาด่วน 3 ระดับ (ขั้นต่ำ / +1 ขั้น / +4 ขั้น), ช่องกรอกเอง, ปุ่มเสนอ ถ้าปิดแล้วแสดงผู้ชนะ |
| เหตุการณ์ของล็อตนี้ | timeline **เฉพาะล็อตนี้**: ประวัติเสนอราคาทุกครั้งจาก server (ของคุณเป็นสีเขียว, รายการบนสุด = ผู้นำ/ผู้ชนะ), ต่อเวลา, ปิดล็อต, เข้าประมูล อัปเดตทันทีเมื่อมี bid ใหม่ |
